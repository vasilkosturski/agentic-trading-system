...
class CandidateStock(BaseModel):
    ...
    symbol: str = Field(min_length=1, max_length=5, description="Stock ticker symbol (e.g., AAPL)")
...
class ResearchResponse(BaseModel):
    ...
    candidates: list[CandidateStock] = Field(
        default_factory=list, description="List of 3-5 stock candidates with their current prices"
    )
    webSources: list[WebSource] = Field(
        default_factory=list, description="List of cited web sources"
    )
    portfolio_context: str = Field(
        default="", description="How portfolio holdings and recent activity influenced research"
    )
...
class TradingDecision(BaseModel):
    ...
    portfolioContext: str = Field(
        default="", description="Current portfolio state and how this trade fits"
    )
    historicalContext: str = Field(
        default="", description="What trading history shows for this stock/sector"
    )
    researchContext: str = Field(
        default="",
        description="Key findings from Market Analyst's research that drove this decision",
    )
    ...
    def validate_consistency(self) -> None:
        ...
        if self.action in (TradeAction.BUY, TradeAction.SELL):
            if not self.symbol or self.quantity <= 0:
                raise ValueError(f"symbol and positive quantity required for {self.action}")
