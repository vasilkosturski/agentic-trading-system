...
public interface AccountPortfolioSnapshotRepository extends JpaRepository<AccountPortfolioSnapshot, Long> {
     ...
     */
    @Query("SELECT aps FROM AccountPortfolioSnapshot aps " + "JOIN FETCH aps.account a "
            + "JOIN FETCH a.agent "
    ...
    @Query("SELECT aps FROM AccountPortfolioSnapshot aps " + "JOIN FETCH aps.account a "
            ...
            + "ORDER BY aps.timestamp DESC")
    List<AccountPortfolioSnapshot> findByTimestampBetween(
    ...
    @Query("SELECT aps FROM AccountPortfolioSnapshot aps " + "JOIN FETCH aps.account a "
            ...
            + "ORDER BY aps.timestamp DESC")
    List<AccountPortfolioSnapshot> findByAgentNameAndDateRange(
