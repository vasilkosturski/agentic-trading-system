...
public interface AccountHoldingRepository extends JpaRepository<AccountHolding, Long> {
    ...
    Optional<AccountHolding> findByAgentNameAndSymbol(
     ...
     */
    @Query(
            "SELECT COALESCE(SUM(ah.quantity), 0) FROM AccountHolding ah WHERE ah.account.agent.name = :agentName AND ah.symbol = :symbol")
    Integer getTotalQuantityByAgentAndSymbol(@Param("agentName") String agentName, @Param("symbol") String symbol);

    /**
     * Get holdings summary for an agent (count of different symbols)
     */
    @Query(
            "SELECT COUNT(DISTINCT ah.symbol) FROM AccountHolding ah WHERE ah.account.agent.name = :agentName AND ah.quantity > 0")
