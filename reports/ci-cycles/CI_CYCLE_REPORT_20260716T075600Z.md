# CI Cycle Report — 2026-07-16T07:56:00Z

## Cycle Summary

Latest cycle in the DB: runs **1101–1104**, started 2026-07-16 06:39:48 UTC. Same
cycle as the previous report (`CI_CYCLE_REPORT_20260716T074939Z.md`) — no new
cycle has landed since; this run re-verifies the same data plus the frontend.
...
| 1102 | 9 | 10 | 2 | first_try | all present | COMPLETED (trade 874) |
| 1103 | 8 | 9 | 2 | first_try | all present | COMPLETED (trade 873) |
| 1104 | 6 | 7 | 2 | first_try | all present | not applicable (HOLD) |
...
- Dashboard (`/`) — loaded, 0 console messages, no failed network requests.
  Portfolio values match the DB snapshots above.
- `/runs/1101` (Ray, HOLD) — Research / Decision sections rendered with full data
  ...
  0 console messages, all API calls 200.
- `/runs/1102` (George, BUY MU) — Research / Decision / Execution sections all
  ...
  trade ID 874). 0 console messages, all API calls 200.
- `GET /api/runs?page=0&limit=5` — newest entry is `runId:1036` (started
  2026-07-09T00:36), `total:1036`. This is exactly 7 days before the current
  date (2026-07-16) and matches the documented `trading.public.display-delay-days=7`
  policy — the public list intentionally withholds runs newer than 7 days. This
  is expected behavior, not a data-freshness bug: the DB and the per-run/
  portfolio-snapshot endpoints are confirmed fresh through runs 1101–1104 above.

Screenshots saved to `reports/ci-cycles/screenshots/` (CI artifact, not committed).

## Drift Comparison vs Previous Report (`CI_CYCLE_REPORT_20260716T074939Z.md`)
...
| Public run-list display delay | OK — capped at run 1036 (7 days old), consistent with the documented 7-day display-delay policy. The previous report mischaracterized this identical behavior as a "REGRESSION"; per the gate's known-behavior list this is correct, expected behavior and is not flagged here. |
